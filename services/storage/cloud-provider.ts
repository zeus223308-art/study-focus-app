...
import { forcePullDriveBackup, pullDriveBackupIfNewer, pushDriveBackup } from '@/services/cloud/drive-sync';

import { getValidAccessToken } from '@/services/cloud/google-session';

import { stampRecoverySettings } from '@/services/storage/auto-recovery';
...
export class CloudStorageProvider implements StorageProvider {
  ...
  async syncAllPending(data: AppData): Promise<AppData> {
    ...
    if (shouldUploadDriveBackup(next)) {
  ...
  async restoreFromCloudBackup(): Promise<AppData | null> {
    ...
    const { data, outcome } = await forcePullDriveBackup(local);

    if (outcome.status === 'pulled') {
      const stamped = await stampRecoverySettings(data);
      await this.local.saveAppData(stamped);
      return stamped;
    }
  ...
  async restoreLocalBackup(): Promise<AppData | null> {
    ...
    return this.local.restoreLocalBackup();
